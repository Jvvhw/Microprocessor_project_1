...
```
레지스터 설정 함수 호출, 시스템 기본 설정

ADC 연속변환 시작, 초기 범위 180도

key 1번을 눌러 align으로 위치정렬이 필요한 시점

반복문으로 들어가게 되면 기본적인 동작이 시작

좌우측 제한값은 업데이트 플래그가 뜨면 업데이트
...
삼각파 증가/감소형 모드 고려하여 그 후 2로 다시 나눠준다. 
